...
/**
  ...
  * @param {String} name Node will get the directory's name from the OS via {fs} -module
  ...
  * @prop {Array | [DirectoryDE || FileDE]} children - array contaning an instantiated {DirectoryDE} || {FileDE}
  *                                                  - object for each and every directory entry inside of it
  *
  */

class DirectoryDE extends IDirectoryEntry {
  constructor( name, parent, level, levelId){
    console.log(name);
    // call super Class first so we can use *this*
    super(name);
    // this.name = name;
    // // the TopDir will have:
    // if(!parent) {
    //   // since we can't overload the constructor, we have to send values along
    //   // level/levelId , however, I like this way... leaves less up to chance
    //   this.parent = null; // no parent
    //   this.level = 0; // the lowest level index
    //   this.levelId = 0; // no siblings***
    // }
    // else // let super class handle these
    // {
      super(parent);
      super(level);
      super(levelId);
