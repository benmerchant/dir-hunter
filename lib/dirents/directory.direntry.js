...
class DirectoryDE extends IDirectoryEntry {
  ...
  constructor(directoryObject){
    ...
    // .name is the path we want to read
    this.children = require('./childArrayBuilder.service').returnTheArray(directoryObject.name);
    /** only directories need level info */
    this.level =  (!directoryObject.homeFlag) ? directoryObject.level : 0;
    this.levelId = (!directoryObject.homeFlag) ? directoryObject.levelId : 0;
  }
}
