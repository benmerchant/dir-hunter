...
class IDirectoryEntry{
  ...
  constructor(genericObject){
    const homeFlag = (genericObject.parent) ? true : false;

    this.name = genericObject.name;

    this.parent = (homeFlag) ? genericObject.parent : null;

    this.level =  (homeFlag) ? genericObject.level : 0;
    this.levelId = (homeFlag) ? genericObject.levelId : 0;
  }
}
